...
void TimeStep( float i_dt )
{
    ...
    float tolerance         = 0.000000000001; // Uncomment and complete for Question 2.5
    ...
    for ( int iter = 0; iter < maxIt; ++iter )
    {
      ...
      float mu = sq(ConstantC)*(sq(i_dt)/sq(DX));
      float aii = 1+2*mu;
      float aij = -mu;
      ...
      for ( int i = 1; i < ArraySize-1; ++i )
      {
          ...
          b[i] = 2*State[m][i] - State[m-1][i];
          //Implement jacobi/Gaus-seidel forumla
          x[n+1][i] = (b[i]-(aij*x[n][i-1]+aij*x[n][i+1]))/aii;
      }
      //////////////////////////////////////////////////
      //// Question 2.5
      ...
      for (i=0; i<ArraySize; ++i){
        num = num + pow(x[n+1][i] - x[n][i],2);
        
      }
      num = sqrt(num);
      
      for(i=0; i<ArraySize; ++i){ 
        den = den + pow(x[n+1][i],2);
      }
      den = sqrt(den);
